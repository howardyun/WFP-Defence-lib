...
def parse_args():
    ...
    parser.add_argument('--is_dump', default=True, type=bool)
...
def generate_defense(para):
    cur_shrunk_super_matrix, now_dir, now_file_path, sampled_slots_upload, sampled_slots_download = para
    ...
    return len(packets), total_pkt, final_end, np.minimum(const.CUTOFF_TIME, times[-1]), packets
...
if __name__ == '__main__':
    ...
    final_band, pre_band, final_time, pre_time, all_packets = zip(*overheads)
    ...
    if is_dump:
        ...
        for i in range(const.MONITORED_SITE_NUM):
            for j in range(const.MONITORED_INST_NUM):
                trace = all_packets[idx]
                timestamps = trace[:, 0] * trace[:, 1]
                if len(timestamps) >= MAX_PKT:
                    timestamps = timestamps[:MAX_PKT]
                else:
                    timestamps = np.pad(timestamps, (0, MAX_PKT - len(timestamps)), constant_values=0)
                all_X.append(timestamps)
                all_Y.append(i)
                idx += 1

        all_X = np.array(all_X)
        all_Y = np.array(all_Y)
        np.savez(os.path.join(out_dir, 'defended-trace.npz'), X=all_X, Y=all_Y)
        print('Saved defended traces to', os.path.join(out_dir, 'defended-trace.npz'))
